...
% --- Load texture image ---
texture = imread('Praktikum_4.jpg'); % ganti dengan file gambar tekstur
...
graphics_toolkit gnuplot  % Ganti renderer agar tampilan lancar

img = imread('bangunan_s5.jpeg');  % Ganti dengan gambar gedung/kotak
gray = rgb2gray(img);

T = [1.2 -0.2 0; 0.3 1.1 0; 30 20 1];  % [a b 0; c d 0; tx ty 1]

% Ubah jadi format transformasi untuk imtransform
tform = maketform('affine', T);

% Transformasi gambar
output = imtransform(gray, tform, 'XData', [1 size(gray,2)], 'YData', [1 size(gray,1)]);

% Tampilkan hasil
subplot(1,2,1); imshow(gray); title('Original');
subplot(1,2,2); imshow(output); title('Affine Transformed');
